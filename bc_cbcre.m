function [b,H,ll]=bc_cbcre(y,x,xm,nc,beta0,maxit,tol,link,family,N1,N2)
...
g=ones(length(b),1);
llold=-10^10;
ll=bc_cbcre_ll(y,x,xm,nc,b,link,family,N1,N2);%ll for initial parameter
while (max(abs(H\g))>tol) && (i1<maxit) && (ll-llold>tol)
    i1=i1+1;
    bold=b;
    llold=ll;
    [~,g,H]=bc_cbcre_lik(y,x,xm,nc,bold,link,family,N1,N2);%get gradient and hessian
    step=H\g;
    ll=bc_cbcre_ll(y,x,xm,nc,bold+step,link,family,N1,N2);%ll for initial parameters+step
...
%         disp(['log-likelihood: ' num2str(ll)]);
        ...
        while ll-llold<0 && i0<8
            step=H\g*(.25^i0);
            ll=bc_cbcre_ll(y,x,xm,nc,bold+step,link,family,N1,N2);%ll for initial parameter+step
        ...
        if i0<8
            [~,g,H]=bc_cbcre_lik(y,x,xm,nc,b,link,family,N1,N2);%get gradient and hessian
        end
%         disp(['iteration: ' num2str(i1)]);
%         disp(['log-likelihood: ' num2str(ll)]);
    end
