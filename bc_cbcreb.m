function [b,a,H,ll]=bc_cbcreb(y,x,xm,nc,beta0,alpha0,maxit,tol,link,N1,d)
...
i1=0;
llold=-10^10;
ll=bc_cbcreb_ll(y,x,xm,nc,b,a,link,N1,d);%ll for initial parameter
stepb=tol+1;
while (max(abs(stepb))>tol) && (i1<maxit) && (ll-llold>tol)
    i1=i1+1;
    bold=b;
    llold=ll;
    [~,gb,Hb,~,~]=bc_cbcreb_lik(y,x,xm,nc,bold,a,link,N1,d);%get gradient and hessian
    stepb=Hb\gb;
    ll=bc_cbcreb_ll(y,x,xm,nc,bold+stepb,a,link,N1,d);%ll for initial parameters+step
    ...
    while (max(abs(stepa))>tol) && (i2<maxit) && (ll-llold2>tol)
        ...
        aold=a;
        llold2=ll;
        [~,~,~,ga,Ha]=bc_cbcreb_lik(y,x,xm,nc,b,aold,link,N1,d);%get gradient and hessian
        stepa=ga./diag(Ha);
        [a,~]=bernsteintrue(aold+stepa,d);%take full step and make sure copula is well-defined
        ll=bc_cbcreb_ll(y,x,xm,nc,b,a,link,N1,d);%ll for initial parameters+step
...
[~,~,~,~,~,H]=bc_cbcreb_lik(y,x,xm,nc,b,a,link,N1,d);%hessian

% cv=bc_cbreb_cv(y,x,nc,b,a,maxit,tol,link,N1,d,ns);%cross validation